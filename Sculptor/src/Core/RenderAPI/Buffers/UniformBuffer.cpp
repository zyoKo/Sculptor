...
namespace Sculptor::Core
{
	...
	VkBuffer UniformBuffer::Create(U64 bufferSize)
	{
		...
		VK_CHECK(vkMapMemory(device, uniformBuffer.GetBufferMemory(), 0, bufferSize, 0, &uniformBufferMapped), "Failed to map memory in Uniform Buffer pointer.")
	...
	{
		const auto* camera = Component::Camera::GetInstance();
